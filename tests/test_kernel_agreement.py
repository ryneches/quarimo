...
class TestCudaBatchArgsBug:
    """
    Regression tests for the cuda_batch_args n_seed bug.

    Root cause
    ----------
    When ``Forest.quartet_topology`` runs on the CUDA backend and a batch
    contains randomly-generated quartets (``batch_needs_rng=True``), the code
    pre-generates all ``bc`` quartets into a scratch device array with a 1D
    kernel, then passes that scratch array as the ``seed_quartets`` input to
    the 2D processing kernel.  In the 2D kernel, every thread qi should read
    from ``seed_quartets[qi]`` because the scratch array holds exactly ``bc``
    quartets at contiguous positions 0…bc-1.

    For this to work, ``n_seed`` must equal ``bc`` so
    ``get_quartet_at_index(qi, seed_quartets, n_seed, ...)`` always takes the
    ``if absolute_idx < n_seed`` branch.

    Bug: ``QuartetKernelArgs.cuda_batch_args`` hardcodes ``self.n_seed``
    (typically 1 — the number of explicit seed quartets in the ``Quartets``
    object) regardless of which seed array is actually passed.  All threads
    qi >= 1 fall through to ``init_xorshift128`` and generate RNG quartets
    that do not match the CPU sequence, corrupting the counts output.

    Symptom observed in production
    --------------------------------
    ``len(df['quartet_idx'].unique()) != len(df.unique())`` — rows that share
    a ``quartet_idx`` (a birthday collision in the CPU random stream) have
    different topology count distributions, because the GPU processed a
    *different* (shifted) quartet at one of those positions.

    Fix
    ---
    ``cuda_batch_args`` must accept a ``proc_n_seed`` parameter that the
    caller passes explicitly, and use it in the returned tuple instead of
    ``self.n_seed``.
    """

    def test_cuda_batch_args_has_proc_n_seed_param(self):
        """cuda_batch_args must expose proc_n_seed as an explicit parameter."""
        import inspect

        from quarimo._kernel_data import QuartetKernelArgs

        sig = inspect.signature(QuartetKernelArgs.cuda_batch_args)
        assert "proc_n_seed" in sig.parameters, (
            "cuda_batch_args is missing the proc_n_seed parameter. "
            "Without it _forest.py cannot pass the correct n_seed for "
            "pre-generated scratch arrays, causing the 2D kernel to re-run "
            "the RNG for threads qi >= self.n_seed."
        )

    def test_cuda_batch_args_returns_proc_n_seed_for_scratch_array(self):
        """When proc_n_seed=bc the returned tuple must contain bc, not self.n_seed."""
        from quarimo._kernel_data import QuartetKernelArgs

        # n_seed=1: typical case — one explicit seed quartet, rest are RNG
        q_args = QuartetKernelArgs(
            seed=np.array([[0, 1, 2, 3]], dtype=np.int32),
            n_seed=1,
            rng_seed=np.uint32(0xDEADBEEF),
            n_quartets=500,
            offset=1,
        )

        bc = 100
        dummy_scratch = np.zeros((bc, 4), dtype=np.int32)

        # Simulate _forest.py with batch_needs_rng=True:
        #   proc_seed = d_quartet_batch (bc quartets pre-generated by 1D kernel)
        #   proc_n_seed = bc
        #   proc_offset = 0
        batch_args = q_args.cuda_batch_args(
            dummy_scratch, proc_n_seed=bc, batch_offset=0, bc=bc
        )
        # batch_args == (seed, n_seed, offset, count, rng_seed)
        returned_n_seed = batch_args[1]

        assert returned_n_seed == bc, (
            f"cuda_batch_args returned n_seed={returned_n_seed}, expected {bc}. "
            f"self.n_seed={q_args.n_seed} was used instead of proc_n_seed={bc}. "
            "The 2D kernel will now call init_xorshift128 for qi >= self.n_seed "
            "instead of reading from the pre-generated scratch array."
        )

    def test_cuda_batch_args_returns_self_n_seed_for_original_array(self):
        """When using the original seed array (no pre-gen), proc_n_seed equals self.n_seed."""
        from quarimo._kernel_data import QuartetKernelArgs
        ...
        dummy_seed = np.zeros((1, 4), dtype=np.int32)

        # batch_needs_rng=False: pass the original seed array, proc_n_seed=self.n_seed
        batch_args = q_args.cuda_batch_args(
            dummy_seed, proc_n_seed=q_args.n_seed, batch_offset=0, bc=1
        )
        returned_n_seed = batch_args[1]

        assert returned_n_seed == q_args.n_seed


class TestCudaRandomQuartetAgreement:
    """
    CUDA backend must match the python backend for randomly-generated quartets.

    The existing TestBackendAgreement.test_counts_python_vs_cuda uses
    ``all_quartets`` (70 explicit seed quartets, offset=0).  That test never
    triggers ``batch_needs_rng=True`` because all quartets live in the seed
    array.

    These tests use ``Quartets.random()`` (offset=1, all quartets RNG-derived)
    to exercise the pre-generation path where the cuda_batch_args n_seed bug
    manifests.
    """

    @pytest.fixture(scope="class")
    def single_forest(self):
        with quiet():
            return Forest(TREES)

    @cuda_skip
    def test_counts_python_vs_cuda_random_quartets(self, single_forest):
        """CUDA counts must equal python counts for random quartets."""
        with quiet():
            q = Quartets.random(single_forest, count=70)
        with silent_benchmark("python"):
            counts_py = single_forest.quartet_topology(q).counts
        with silent_benchmark("cuda"):
            counts_cuda = single_forest.quartet_topology(q).counts
        np.testing.assert_array_equal(
            counts_py,
            counts_cuda,
            err_msg=(
                "CUDA and python disagree for random quartets. "
                "Likely cause: cuda_batch_args n_seed bug — the 2D kernel "
                "receives self.n_seed instead of bc, re-running the RNG "
                "for qi >= self.n_seed instead of reading the scratch array."
            ),
        )

    @cuda_skip
    def test_steiner_python_vs_cuda_random_quartets(self, single_forest):
        """CUDA Steiner sums must match python for random quartets."""
        with quiet():
            q = Quartets.random(single_forest, count=70)
        with silent_benchmark("python"):
            steiner_py = single_forest.quartet_topology(q, steiner=True).steiner
        with silent_benchmark("cuda"):
            steiner_cuda = single_forest.quartet_topology(q, steiner=True).steiner
        np.testing.assert_allclose(steiner_py, steiner_cuda, rtol=1e-8, atol=1e-10)
